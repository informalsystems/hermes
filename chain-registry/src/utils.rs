...
pub async fn fetch_data<'a, T : FileName>(chain_name: &'a str) -> Result<String, RegistryError> {
    match Builder::new()
        .scheme(PROTOCOL)
        .authority(HOST)
        .path_and_query(format!("{}/{}/{}/{}", REGISTRY_PATH, REF, chain_name, T::file_name()).as_str())
        .build()
    {
        Ok(url) => {
