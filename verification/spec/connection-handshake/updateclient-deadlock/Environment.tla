...
VARIABLES
    ...
    relayToA,       \* Flag, signaling that the env. can relay a msg. to A.
    relayToB        \* Flag, signaling that the env. can relay a msg. to B.
...
(* Updates the client on one of the chains, and updates the relay flag for that
    chain (signaling that the env. is ready to relay a message to that chain).
...
UpdateChainClient ==
    \/ /\ chmA!CanUpdateClient(storeChainB.latestHeight)
       ...
       /\ ~ relayToA
       /\ relayToA' = TRUE (* We can now relay to A. *)
       /\ UNCHANGED<<relayToB,storeChainB, outBufChainA, outBufChainB, inBufChainA, inBufChainB>>
    \/ /\ chmB!CanUpdateClient(storeChainA.latestHeight)
       /\ chmB!UpdateClient(storeChainA.latestHeight)
       /\ ~ relayToB
       /\ relayToB' = TRUE (* We can now relay to B. *)
       /\ UNCHANGED<<relayToA,storeChainA, outBufChainA, outBufChainB, inBufChainA, inBufChainB>>
...
RelayNextEnv ==
    \/ LET msg == Head(outBufChainA)
       ...
       IN /\ RelayMessage(outBufChainA, inBufChainB)
          ...
          /\ UNCHANGED<<relayToA,storeChainA, outBufChainB, inBufChainA>>
    \/ LET msg == Head(outBufChainB)
       ...
       IN /\ RelayMessage(outBufChainB, inBufChainA)
          ...
          /\ UNCHANGED<<relayToB,storeChainB, outBufChainA, inBufChainB>>
 ...
 *)
NextEnv ==
     (* Update the clients on the corresponding chain. *)
    \/ /\ (outBufChainA # <<>> \/ outBufChainB # <<>>)
       /\ UpdateChainClient
    (* Clients are already updated, we relay the message. *)
    \/ RelayNextEnv
...
Next ==
    ...
    \/ ICS3ImpossibleToAdvance
    \/ NextEnv
    \/ chmA!Next /\ UNCHANGED <<chainBVars,relayToA,relayToB>>
    \/ chmB!Next /\ UNCHANGED <<chainAVars,relayToA,relayToB>>
...
\* Last modified Thu May 21 08:44:06 CEST 2020 by adi
