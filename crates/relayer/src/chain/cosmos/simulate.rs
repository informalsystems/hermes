...
pub async fn send_tx_simulate(grpc_address: &Uri, tx: SignedTx) -> Result<SimulateResponse, Error> {
    ...
    prost::Message::encode(&raw_tx, &mut tx_bytes)
        .map_err(|e| Error::protobuf_encode(String::from("Transaction"), e))?;

    #[allow(deprecated)]
    let req = SimulateRequest {
        tx: Some(raw_tx), // needed for simulation to go through with Cosmos SDK <  0.43
