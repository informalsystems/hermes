...
impl Runnable for EvidenceCmd {
    fn run(&self) {
        ...
        let mut chain = CosmosSdkChain::bootstrap(chain_config, rt).unwrap();
        ...
        //         Ok(None)
        //     };
        let res = monitor_equivocation(&mut chain);
...
fn monitor_equivocation(chain: &mut CosmosSdkChain) -> eyre::Result<Option<IbcEvent>> {
    ...
    for _h in 0..num_blocks - 1 {
        ...
        equivocation_handling(chain, height)?;
            ...
            Ok(event_batch) => {
                for event_with_height in &event_batch.events {
                    if let IbcEvent::NewBlock(new_block) = &event_with_height.event {
                        debug!("{:?}", new_block);
                        equivocation_handling(chain, new_block.height)?;
                    }
                }
            }
            Err(e) => {
...
fn equivocation_handling(chain: &CosmosSdkChain, height: Height) -> eyre::Result<()> {
    let tm_height = tendermint::block::Height::try_from(height.revision_height()).unwrap();
    let evidence_list = chain
        .block_on(chain.rpc_client.block(tm_height))
        .unwrap()
        .block
        .evidence;
