...
pub struct RegisterCounterpartyPayeeCmd {
    ...
    #[clap(
        ...
        help = """ \
            ...
            Note that there exists a configuration parameter `auto_register_counterparty_payee` \
            that can be enabled in order to have Hermes automatically register the counterparty \
            payee on the destination chain to the relayer's address on the source chain. This \
            option can be used for simple configuration of the relayer to receive fees for \
           relaying RecvPackets on fee-enabled channels.\
        """
    )]
