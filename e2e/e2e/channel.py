...
def handshake(
    ...
    expected_state = str({'Open': 'NotUpgrading'})
    if str(a_chan_end.state) != str({'Open': 'NotUpgrading'}):
        l.error(
            f"Channel end with id {a_chan_id} on chain {side_a} is not in {expected_state} state, got: {a_chan_end.state}")
        exit(1)

    b_chan_end = query_channel_end(c, side_b, port_id, b_chan_id)
    print(b_chan_end.state)
    if str(b_chan_end.state) != str({'Open': 'NotUpgrading'}):
        l.error(
            f'Channel end with id {b_chan_id} on chain {side_b} is not in {expected_state} state, got: {b_chan_end.state}')
        exit(1)
