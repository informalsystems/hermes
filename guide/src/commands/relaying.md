...
The `start` command can be used to send packet transactions triggered by IBC packet
events that occur for all channels between the configured chains.
...
To start the relayer, invoke the `start` command. Note that it does not require any options.
