...
## Recheck
When relaying packets, Hermes may send up multiple transactions to the full node's mempool. Hermes uses `broadcast_tx_sync` RPC that does basic verification and returns the Tx hash back. 
...
During peak periods it is possible that not all Tx-es in the mempool are included in a block. In order for new transactions to be accepted along the pending Tx-es, the full node must be configured with `recheck = true`. Otherwise, Hermes may get the following error:
```
...
## Minimum Gas Price
Hermes sends transactions using the `gas_price` parameter from the chain section in the configuration file. The full node will not accept any Tx with a gas price smaller than what it is configured for the applications (`app.minimum-gas-prices`) and Hermes will log an `insufficient fees` error:
...
### Fix
Ensure that the `gas_price.price` parameter in the chain section of the configuration file is greater than or equal to the `app.minimum-gas-prices`.

## Out of Gas
Before Hermes sends a transaction it evaluates the gas it will require by calling the `SimulateTx()` gRPC. This returns the amount of gas that the Tx requires given the application state at the time of the call. It is possible that by the time the transaction is sent and checked by the application the amount of gas required to be slightly higher. To handle this, after simulation the Tx is sent with an adjusted gas: `gas_multiplier * simulated_gas`.

If the `gas_multiplier` is too low, e.g. `1.0`, then Hermes may get the following error:
```
...
### Fix
Ensure that the `gas_multiplier` parameter in the chain section of the configuration file is configured such that it allows some increase over the simulated gas. A good value is for example `1.1`.
...
## Historical Entries
Presence of recent consensus states on a chain is required when processing some connection handshake messages (`MsgConnectionOpenTry` and `MsgConnectionOpenAck`) that require proof verification that the counterparty chain has a valid client for this chain.
If the chain is configured with `genesis.app_state.staking.params.historical_entries = 0`, then the chain will not store historical consensus state information and Hermes cannot open IBC connections with this chain. It will report an error message will look like this:
...
The chain genesis misconfiguration is caught when health check runs, e.g. `hermes health-check`, `hermes start`. A warning message is printed.
...
Hermes sends the update if:`header.timestamp <= dst_timestamp + max_clock_drift`
...
Hermes uses it's config `max_block_delay` to compute the block delay.
If Hermes `max_block_delay > genesis.app_state.ibc.connection_genesis.params.max_expected_time_per_block` then Hermes may send a packet that is received too early on the destination chain.
...
```
- endure that the genesis max_expected_time_per_block parameter is 2 or 3 times the average block time which can be estimates from the `tendermint.consensus.timeout*` parameters. A good estimation is:
```
