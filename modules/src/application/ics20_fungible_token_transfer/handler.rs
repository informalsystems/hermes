    ...
    for any_msg in messages {
        ...
        match any_msg.type_url.as_str() {
            ...
            transfer::TYPE_URL => {
                ...
                let mut output = dispatch(&mut ctx_interim, domain_msg)?;
                res.append(&mut output.events);
            }
            _ => Err(Kind::UnknownMessageTypeURL(any_msg.type_url)),
        }?;
        // // Process the message and accumulate any events that were generated.
...
pub fn dispatch<Ctx>(_ctx: &Ctx, msg: MsgTransfer) -> Result<HandlerOutput<()>, Error>
// //() - > SendTransferResult ?
where
    Ctx: ICS20Context,
{
    ...
    // 		return nil, err
    // 	}
    let handler_output = relay::send_transfer::sendTransfer(ctx, msg)
