...
impl TryFrom<RawMsgRecvPacket> for MsgRecvPacket {
	...
	fn try_from(raw_msg: RawMsgRecvPacket) -> Result<Self, Self::Error> {
		let proofs = Proofs::new(
			raw_msg.proof_commitment.try_into().map_err(Error::invalid_proof)?,
			None,
			None,
			None,
	...
	fn msg_recv_packet_try_from_raw() {
		...
		let tests: Vec<Test> = vec![
			...
			Test {
				...
				raw: RawMsgRecvPacket { proof_commitment: Vec::new(), ..default_raw_msg.clone() },
				want_pass: false,
			},
			Test {
				name: "Missing proof height".to_string(),
				raw: RawMsgRecvPacket { proof_height: None, ..default_raw_msg.clone() },
				want_pass: false,
			},
			Test {
				name: "Empty signer".to_string(),
				raw: RawMsgRecvPacket { signer: get_dummy_bech32_account(), ..default_raw_msg },
				want_pass: true,
			},
		];
