...
impl MsgAcknowledgement {
	pub fn new(
		...
		Self { packet, acknowledgement, proofs, signer }
	}
...
impl TryFrom<RawMsgAcknowledgement> for MsgAcknowledgement {
	...
	fn try_from(raw_msg: RawMsgAcknowledgement) -> Result<Self, Self::Error> {
		let proofs = Proofs::new(
			raw_msg.proof_acked.try_into().map_err(Error::invalid_proof)?,
			None,
			None,
			None,
...
pub mod test_util {
	use ibc_proto::ibc::core::{
		channel::v1::MsgAcknowledgement as RawMsgAcknowledgement, client::v1::Height as RawHeight,
	};
	...
	fn msg_acknowledgment_try_from_raw() {
		...
		let tests: Vec<Test> = vec![
			...
			Test {
				...
				raw: RawMsgAcknowledgement { packet: None, ..default_raw_msg.clone() },
				want_pass: false,
			},
			Test {
				name: "Missing proof height".to_string(),
				raw: RawMsgAcknowledgement { proof_height: None, ..default_raw_msg.clone() },
				want_pass: false,
			},
			Test {
			...
			Test {
				...
				raw: RawMsgAcknowledgement { proof_acked: Vec::new(), ..default_raw_msg },
				want_pass: false,
			},
		];
