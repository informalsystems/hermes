...
use crate::{
	core::{
		...
		ics04_channel::{error::Error, packet::Sequence, Version},
...
impl IdentifiedChannelEnd {
	pub fn new(port_id: PortId, channel_id: ChannelId, channel_end: ChannelEnd) -> Self {
		IdentifiedChannelEnd { port_id, channel_id, channel_end }
	}
}
...
impl TryFrom<RawChannel> for ChannelEnd {
	...
	fn try_from(value: RawChannel) -> Result<Self, Self::Error> {
		...
		let remote = value.counterparty.ok_or_else(Error::missing_counterparty)?.try_into()?;
...
impl From<ChannelEnd> for RawChannel {
	fn from(value: ChannelEnd) -> Self {
		RawChannel {
			...
			connection_hops: value.connection_hops.iter().map(|v| v.as_str().to_string()).collect(),
...
impl ChannelEnd {
	...
	pub fn new(
		...
		Self { state, ordering, remote, connection_hops, version }
	}
...
impl TryFrom<RawCounterparty> for Counterparty {
	...
	fn try_from(value: RawCounterparty) -> Result<Self, Self::Error> {
		let channel_id = Some(value.channel_id)
			...
			.transpose()
			.map_err(Error::identifier)?;
		Ok(Counterparty::new(value.port_id.parse().map_err(Error::identifier)?, channel_id))
	}
}
...
impl From<Counterparty> for RawCounterparty {
	fn from(value: Counterparty) -> Self {
		RawCounterparty {
			...
			channel_id: value.channel_id.map_or_else(|| "".to_string(), |v| v.to_string()),
		}
	}
}
...
mod tests {
	...
	fn channel_end_try_from_raw() {
		...
		let empty_raw_channel_end = RawChannel { counterparty: None, ..raw_channel_end.clone() };
		...
		let tests: Vec<Test> = vec![
			...
			Test {
				...
				params: RawChannel { state: -1, ..raw_channel_end.clone() },
				want_pass: false,
			},
			Test {
				name: "Raw channel end with incorrect ordering".to_string(),
				params: RawChannel { ordering: -1, ..raw_channel_end.clone() },
				want_pass: false,
			},
			Test {
