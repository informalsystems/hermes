...
impl From<Version> for RawVersion {
	fn from(value: Version) -> Self {
		Self { identifier: value.identifier, features: value.features }
	}
}
...
mod tests {
	...
	fn bad_versions_features() -> Vec<RawVersion> {
		vec![RawVersion { identifier: "2".to_string(), features: vec!["".to_string()] }]
			.into_iter()
			.collect()
	}
	...
	fn overlapping() -> (Vec<Version>, Vec<Version>, Version) {
		(
			vec![
				Version::default(),
				Version { identifier: "3".to_string(), features: Vec::new() },
				Version { identifier: "4".to_string(), features: Vec::new() },
			]
			.into_iter()
			.collect(),
			vec![
				Version { identifier: "2".to_string(), features: Vec::new() },
				Version { identifier: "4".to_string(), features: Vec::new() },
				Version { identifier: "3".to_string(), features: Vec::new() },
			]
			.into_iter()
	...
	fn disjoint() -> (Vec<Version>, Vec<Version>) {
		(
			vec![Version { identifier: "1".to_string(), features: Vec::new() }]
				.into_iter()
				.collect(),
			vec![Version { identifier: "2".to_string(), features: Vec::new() }]
				.into_iter()
				.collect(),
		)
	}
	...
	fn verify() {
		...
		let tests: Vec<Test> = vec![
			...
			Test { name: "Bad versions empty".to_string(), versions: Vec::new(), want_pass: true },
		];

		for test in tests {
			let versions =
				test.versions.into_iter().map(Version::try_from).collect::<Result<Vec<_>, _>>();
