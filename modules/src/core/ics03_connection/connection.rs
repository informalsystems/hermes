...
use core::{fmt, str::FromStr, time::Duration, u64};
...
impl IdentifiedConnectionEnd {
	pub fn new(connection_id: ConnectionId, connection_end: ConnectionEnd) -> Self {
		IdentifiedConnectionEnd { connection_id, connection_end }
	}
...
impl From<IdentifiedConnectionEnd> for RawIdentifiedConnection {
	fn from(value: IdentifiedConnectionEnd) -> Self {
		RawIdentifiedConnection {
			...
			versions: value.connection_end.versions.iter().map(|v| From::from(v.clone())).collect(),
...
impl TryFrom<RawConnectionEnd> for ConnectionEnd {
	...
	fn try_from(value: RawConnectionEnd) -> Result<Self, Self::Error> {
		...
		Ok(Self::new(
			...
			value.counterparty.ok_or_else(Error::missing_counterparty)?.try_into()?,
			value
				.versions
				.into_iter()
...
impl From<ConnectionEnd> for RawConnectionEnd {
	fn from(value: ConnectionEnd) -> Self {
		RawConnectionEnd {
			client_id: value.client_id.to_string(),
			versions: value.versions.iter().map(|v| From::from(v.clone())).collect(),
...
impl ConnectionEnd {
	pub fn new(
		...
		Self { state, client_id, counterparty, versions, delay_period }
	}
...
impl Counterparty {
	pub fn new(
		...
		Self { client_id, connection_id, prefix }
	}
