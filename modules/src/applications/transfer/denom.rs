...
use ibc_proto::{
	...
	ibc::applications::transfer::v1::DenomTrace as RawDenomTrace,
};
...
impl<'a> TryFrom<Vec<&'a str>> for TracePath {
	...
	fn try_from(v: Vec<&'a str>) -> Result<Self, Self::Error> {
		...
		for (pos, (port_id, channel_id)) in id_pairs.rev().enumerate() {
			...
			trace.push(TracePrefix { port_id, channel_id });
		}
...
pub fn is_receiver_chain_source(
	...
	// B: receiver chain in this transfer, port "transfer" and channel "c2a" (to A)
	//
	// If B had originally sent the token in a previous tranfer, then A would have stored the token
...
impl From<PrefixedDenom> for RawDenomTrace {
	fn from(value: PrefixedDenom) -> Self {
		Self { path: value.trace_path.to_string(), base_denom: value.base_denom.to_string() }
	}
}
...
impl<D: ToString> From<Coin<D>> for RawCoin {
	fn from(coin: Coin<D>) -> RawCoin {
		RawCoin { denom: coin.denom.to_string(), amount: coin.amount.to_string() }
	}
}

impl From<BaseCoin> for PrefixedCoin {
	fn from(coin: BaseCoin) -> PrefixedCoin {
		PrefixedCoin { denom: coin.denom.into(), amount: coin.amount }
	}
}
...
mod tests {
	...
	fn test_denom_validation() -> Result<(), Error> {
		...
		assert!(PrefixedDenom::from_str("transfer/").is_err(), "single trace");
		assert!(PrefixedDenom::from_str("transfer/atom").is_err(), "single trace with base denom");
		assert!(
		...
		assert!(PrefixedDenom::from_str("(transfer)/channel-0/uatom").is_err(), "invalid port");
		assert!(PrefixedDenom::from_str("transfer/(channel-0)/uatom").is_err(), "invalid channel");
	...
	fn test_trace_path() -> Result<(), Error> {
		...
		assert_eq!(TracePath::from_str("transfer/channel-0/transfer/channel-1")?, trace_path);
		assert_eq!(TracePath(vec![prefix_1.clone(), prefix_2.clone()]), trace_path);
