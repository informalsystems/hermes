...
use crate::ics02_client::{client_type::ClientType, error::Kind};
use crate::{
    ics02_client::{client_def::AnyHeader, error::Error},
    try_from_raw::TryFromRaw,
};
...
impl TryFromRaw for MockHeader {
    ...
    fn try_from(raw: Self::RawType) -> Result<Self, Self::Error> {
        let proto_height = raw.height.ok_or_else(|| Kind::InvalidRawHeader)?;
