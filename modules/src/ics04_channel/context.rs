...
use crate::ics24_host::identifier::{ConnectionId,PortId,ChannelId};
//use crate::Height;
...
pub trait ChannelKeeper {
    fn store_channel_result(&mut self, result: ChannelResult) -> Result<(), Error> {
        match result.channel_end.state() {
            State::Init => {
                self.store_channel(&(result.port_id.clone(),result.channel_id.clone()), &result.channel_end)?;

                self.store_nextsequence_send(&(result.port_id.clone(),result.channel_id.clone()),1)?;

                self.store_nextsequence_recv(&(result.port_id.clone(),result.channel_id.clone()),1)?;

                self.store_nextsequence_ack(&(result.port_id.clone(),result.channel_id.clone()),1)?;
