...
pub fn process(
    ...
    let result = if dest_channel_end.order_matches(&Order::Ordered) {
        let next_seq_recv = ctx
            ...
            .ok_or(error::missing_next_recv_seq_error())?;

        if packet.sequence != next_seq_recv {
            return Err(
                error::invalid_packet_sequence_error(packet.sequence, next_seq_recv).into(),
            );
        }
        ...
        match packet_rec {
            Some(_receipt) => {
                return Err(error::packet_already_received_error(packet.sequence).into())
            }
            None => {
