...
pub mod test_util {
    ...
    /// Returns a dummy `RawMsgConnectionOpenTry` with parametrized proof height (the height, on the
    /// source chain, at which this chain produced the proof) and consensus height (the height of
    /// destination chain, which a client on the source chain stores).
