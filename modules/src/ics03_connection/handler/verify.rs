    ...
    if let Some(expected_client_state) = client_state {
        verify_client_proof(
            ...
            proofs
                .client_proof()
                .as_ref()
                .ok_or(error::null_client_proof_error())?,
        )?;
    }
...
pub fn verify_client_proof(
    ...
    Ok(client_def
        ...
        .map_err(|_| {
            error::client_state_verification_failure_error(connection_end.client_id().clone())
        })?)
}
...
pub fn verify_consensus_proof(
    ...
    let expected_consensus = ctx
        ...
        .ok_or_else(|| error::missing_local_consensus_state_error())?;
    ...
    Ok(client
        ...
        .map_err(|_| error::consensus_state_verification_failure_error(proof.height()))?)
}
