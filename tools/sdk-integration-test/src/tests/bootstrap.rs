...
fn cosmos_integration_tests() -> Result<(), Error> {
    ...
    let (relayer_v1_driver, _driver) = runtime.runtime.clone().block_on(async move {

        let driver: hermes_cosmos_integration_tests::contexts::binary_channel::test_driver::CosmosBinaryChannelTestDriver = setup.build_driver().await?;

        let chain_driver_a = driver.chain_driver_at(Index::<0>);
        let chain_a = chain_driver_a.chain();
        let chain_b = chain_driver_a.chain();
        ...
        test_config.chain_command_paths = vec!(bootstrap.chain_command_path.clone().to_string_lossy().into());
        ...
        config.chains.push(ibc_relayer::config::ChainConfig::CosmosSdk(chain_a.chain_config.clone()));
        config.chains.push(ibc_relayer::config::ChainConfig::CosmosSdk(chain_b.chain_config.clone()));
        ...
        <Result<(RelayerDriver, hermes_cosmos_integration_tests::contexts::binary_channel::test_driver::CosmosBinaryChannelTestDriver), Error>>::Ok((relayer_v1_driver, driver))
    })?;

    relayer_v1_driver.spawn_supervisor()?;
