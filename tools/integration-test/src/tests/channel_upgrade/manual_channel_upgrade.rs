...
//!
//! - `ChannelUpgradeAckHandshake` tests that after the `ChanUpgradeAck` and
//!   `ChanUpgradeTry` steps, the state is correctly set to (OPEN, TRYUPGRADE),
//!   that the fields updated by the channel upgrade have been modified on the
//!   Channel End which is OPEN, but not on the other Channel End.
