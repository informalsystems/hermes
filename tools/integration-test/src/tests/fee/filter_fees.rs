...
use ibc_relayer::config::filter::{ChannelPolicy, FeeInformation, FeesFilters, FilterPattern};
...
/*#[test]
fn test_filter_incentivized_fees_ack_only_relayer() -> Result<(), Error> {
    run_binary_channel_test(&FilterIncentivizedFeesAckOnlyRelayerTest)
}*/
...
/*struct FilterIncentivizedFeesAckOnlyRelayerTest;
...
impl BinaryChannelTest for FilterIncentivizedFeesAckOnlyRelayerTest {
    fn run<ChainA: ChainHandle, ChainB: ChainHandle>(
        &self,
        _config: &TestConfig,
        _relayer: RelayerDriver,
...

        {
            ...
            let recv_fee = random_u128_range(300, 400);
            let ack_fee_fail = 49u128;
            let timeout_fee = random_u128_range(100, 200);

            let total_sent_fail = send_amount + recv_fee + ack_fee_fail + timeout_fee;
            ...
            chain_driver_a.ibc_token_transfer_with_fee(
                ...
                &user_b.address(),
                &denom_a.with_amount(send_amount).as_ref(),
                &denom_a.with_amount(recv_fee).as_ref(),
                &denom_a.with_amount(ack_fee_fail).as_ref(),
                &denom_a.with_amount(timeout_fee).as_ref(),
                Duration::from_secs(60),
            )?;
...

        {
            ...
            let recv_fee = random_u128_range(300, 400);
            let ack_fee_success = 50u128;
            let timeout_fee = random_u128_range(100, 200);

            let total_sent_success = send_amount + recv_fee + ack_fee_success + timeout_fee;
            ...
            chain_driver_a.ibc_token_transfer_with_fee(
                ...
                &denom_a.with_amount(send_amount).as_ref(),
                &denom_a.with_amount(recv_fee).as_ref(),
                &denom_a.with_amount(ack_fee_success).as_ref(),
                &denom_a.with_amount(timeout_fee).as_ref(),
                Duration::from_secs(60),
            )?;
            ...
            chain_driver_a.assert_eventual_wallet_amount(
                &relayer_a.address(),
                &(relayer_balance_a + recv_fee + ack_fee_success).as_ref(),
            )?;
        }
