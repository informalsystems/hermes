use ibc_test_framework::framework::next::chain::{
    CanSpawnRelayer, HasContextId, HasTwoChains, HasTwoChannels,
};
...
#[test]
fn test_current_relay_transfer() -> Result<(), Error> {
    run_binary_channel_test(&IbcTransferTest)
}
...
impl BinaryChannelTest for IbcTransferTest {
        ...
        Context: HasTwoChains + HasTwoChannels + CanSpawnRelayer + HasContextId,
    {
        ...
        let channel = context.channel_a_to_b();
...
/*#[async_trait]
impl<Context> TestRunner<Context> for IbcTransferTest
where
    Context: HasTwoChains + HasTwoChannels + CanSpawnRelayer + Send + Sync,
{
    async fn run_test(context: &Context) -> Result<(), Error> {

        context.spawn_relayer()?;
