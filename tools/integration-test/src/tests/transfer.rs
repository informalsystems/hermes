...
impl BinaryChannelTest for IbcTransferTest {
    fn run<ChainA: ChainHandle, ChainB: ChainHandle>(
        ...
        relayer.with_supervisor(|| {
            ...
            let a_to_b_amount_each = random_u64_range(1000, 5000);
            let a_to_b_amount_total = a_to_b_amount_each * TRANSFER_COUNT;

            info!(
                "Sending IBC transfer from chain {} to chain {} with amount of {} {} x {}",
                chains.chain_id_a(),
                chains.chain_id_b(),
                a_to_b_amount_each,
                denom_a,
            ...
            info!(
                ...
                a_to_b_amount_total, denom_b
            );

            chains.node_a.chain_driver().assert_eventual_wallet_amount(
                &wallet_a.address(),
                balance_a - a_to_b_amount_total,
                &denom_a,
            )?;
            ...
            let b_to_a_amount = random_u64_range(500, a_to_b_amount_total);
            ...
            chains.node_b.chain_driver().assert_eventual_wallet_amount(
                &wallet_b.address(),
                a_to_b_amount_total - b_to_a_amount,
                &denom_b.as_ref(),
            )?;
