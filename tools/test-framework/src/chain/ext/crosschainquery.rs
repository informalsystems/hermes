...
use crate::prelude::{assert_eventually_succeed, ChainDriver};
    ...
    fn assert_pending_cross_chain_query(&self) -> Result<(), Error> {
        assert_eventually_succeed(
            ...
            || {
                ...
                let request_sent = output
                    .pending_queries
                    .first()
                    .map(|query| query.request_sent);

                if let Some(false) = request_sent {
    ...
    fn assert_processed_cross_chain_query(&self) -> Result<(), Error> {
        assert_eventually_succeed(
            ...
            || {
                ...
                if !output.pending_queries.is_empty() {
