...
mod tests {
    ...
    fn test_query_packet_acks() {
        assert_eq!(
            QueryPacketAcknowledgementsCmd{chain_id: ChainId::from_string("chain_id"), port_id: PortId::from_str("port_id").unwrap(), channel_id: ChannelId::from_str("channel-07").unwrap()},
            QueryPacketAcknowledgementsCmd::parse_from(&["test", "--chain", "chain_id", "--port", "port_id", "--chan", "channel-07"])
        )
    }

    #[test]
    fn test_query_packet_acks_no_chan() {
        assert!(QueryPacketAcknowledgementsCmd::try_parse_from(&["test", "--chain", "chain_id", "--port", "port_id"]).is_err())
    }

    #[test]
    fn test_query_packet_acks_no_port() {
        assert!(QueryPacketAcknowledgementsCmd::try_parse_from(&["test", "--chain", "chain_id", "--chan", "channel-07"]).is_err())
    }

    #[test]
    fn test_query_packet_acks_no_chain() {
        assert!(QueryPacketAcknowledgementsCmd::try_parse_from(&["test", "--port", "port_id", "--chan", "channel-07"]).is_err())
    }
}
