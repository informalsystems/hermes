...
info "Spawning duplicate chain ibc-1"
cp -r data/ibc-1 data/sybil
echo '{"height": "0","round": 0,"step": 0,"signature":"","signbytes":""}' > data/sybil/data/priv_validator_state.json
echo '{"priv_key":{"type":"tendermint/PrivKeyEd25519","value":"tj55by/yYwruSz4NxsOG9y9k2WrPvKLXKQdz/9jL9Uptmi647OYpcisjwf92TyA+wCUYVDOgW7D53Q+638l9/w=="}}' > data/sybil/config/node_key.json

ARGS="--address tcp://$CHAIN_PREFIX:29655 \
    --rpc.laddr tcp://$CHAIN_PREFIX:29658 \
    --grpc.address $CHAIN_PREFIX:29091 \
    --p2p.laddr tcp://$CHAIN_PREFIX:29656 \
    --log_level info \
    --grpc-web.enable=false"

gaiad $ARGS --home data/sybil start &> data/sybil/logs &
