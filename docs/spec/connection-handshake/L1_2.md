...
{ 
  ...
  expectedConn = NewConnectionEnd('INIT', remote, local)
...
This function is analogous to `verifyMembership` of the [ICS 023](#references), except it must additionally perform two actions: serialize the expected data of the proof, and to extract from the client state the path to the root of the tree.
