...
- **[ICS3-CommAssumption-1-RWIntegrity]** If some process returns a connection `e` from `getConnection(i)` for some module, then the module previously invoked `setConnection(i, e)`.
...
Implementation remarks:
  - to ensure termination of the ICS3 protocol, this handler must execute at least once (either one of the chains or both)
