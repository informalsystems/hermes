...
{ 
    ...
    abortTransactionUnless(
        current == nil || 
        current.state == 'INIT' || 
        matchingEnds(connectionEnd, current))

    // Proofs verification.
    // Generic proof check: The local client exists and should be updated with the
    // height which the two proofs target (proofsHeight).
    client = clientState(local.clientIdentifier)
    abortTransactionUnless(client != nil)
    clientConsensusState = clientConsensusState(local.clientIdentifier, proofsHeight)
    ...
    abortTransactionUnless(client.verifyProof(
        ...
        connectionPath(local.connectionIdentifier), expectedRemoteConnection))

    // Client proof verification ensurea state consistency: the remote
...
{ 
    ...
    // Client proof verification ensures state consistency
    expectedRemoteClientConsensus = localConsensusState(remoteClientProof.height)
    abortTransactionUnless(remoteClientProof.height <= getCurrentHeight())
    abortTransactionUnless(
        client.verifyProof(
    ...
    remoteConnectionProof ConnProof)
{
    ...
    // Get local client and its consensus state at proofs height
    client = clientState(local.clientIdentifier)
    abortTransactionUnless(client != nil)
    clientConsensusState = clientConsensusState(local.clientIdentifier, proofsHeight)
    abortTransactionUnless(clientConsensusState != nil)
...
This function executes from a client context and handles the cryptographic verification of a proof.
It expects four arguments: a consensus state, a proof, a path, and the expected data of the proof.
This function is analogous to `verifyMembership` of the [ICS 023](#references), except it must additionally get the commitment root from the consensuss state and perform serialization the expected data.
