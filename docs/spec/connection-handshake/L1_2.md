...
type ConnProof struct {
  connectionEnd ConnectionEnd // The connection that the remote party claims to have.
}
```
...
__A connection handshake enables two modules (each sitting on a chain) to agree on a consistent view of their chain state, to verify each other's state, and to allocate a new connection.__
...
<!-- - The integrity property prevents executions where a connection is spouriously opened, e.g., some third-party (a malicious chain) fabricates and leads two chains to open a connection  -->
                    ...
                    remoteClientProof ClientProof)
{
  ...
  // TODO: Add client id parameter in `localClientHasConsensusHeight`.
  // TODO: Document this function in the helpers section below.
...
Implementation remarks:
  - the `validate` predicate should also check the field `connectionEnd.version`.
                    ...
                    remoteClientProof ClientProof)
{
  ...
  // unless it is for a version that the local module agrees with.
  abortTransactionUnless(getCompatibleVersions().contains(remoteVersion))
