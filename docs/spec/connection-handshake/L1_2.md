...
In other words, if a connection handshake finishes, the two parties have a consistent view of this connection. We say that Alice has one end of the connection, label `c1`, and Bob stores the other end of the same connection, label `c2`.
...
If a module establishes a `ConnectionEnd` `e`, no handshake has previously finished for `e`.

- **[ICS3-Proto-2-ConnectionIntegrity]**
If any two modules establish a connection `e`, then either one module or the other or both modules accepted (i.e., initialized with) `e`.
...
- The `ClientProof` type helps guarantee the state consistency property, by asserting that the client of a certain chain is updated to a certain height of the remote chain.

- Note that property [ICS3-Proto-3-StateConsistency] works both ways.
...
func ConnInitHandler(local ConnectionParty, remote ConnectionParty)
{
  ...
  connectionEnd = NewConnectionEnd(
    'INIT', local, remote, getCompatibleVersions())
...
Preconditions:
  - The connection local end (the input `local` argument) should be valid, i.e., a syntactically correct.
...
{ 
  ...
  connectionEnd = newConnectionEnd(
      'TRYOPEN', local, remote, pickVersion(remoteVersions));

  // Uniqueness check.
  // Verify that either there is no connection OR it is in INIT state (i.e., 
  // the Init handler previously executed with these same parameters).
  currnt = getConnection(local.connectionIdentifier)
  if currnt != nil {
    abortTransactionUnless(currnt.state = 'INIT')
  }
  abortTransactionUnless(currnt == nil || matchingEnds(connectionEnd, currnt))

  expectedConn = newConnectionEnd('INIT', remote, local, remoteVersions)
  expectedClient = localConsensusState(remoteClientProof.height)

  // Proofs generic verification.
  // Generic proof check: The local client should already be updated with the
  // height which the two proofs target (proofsHeight).
  localClientState = clientConsensusState(local.clientIdentifier, proofsHeight)
  abortTransactionUnless(localClientState != nil)

  // Connection proof specific verification. See `verifyProof`.
  abortTransactionUnless(verifyProof(
      localClientState, remoteConnectionProof,
      connectionPath(local.connectionIdentifier), expectedConn))

  // Client proof verification. Helps ensure state consistency: The remote 
  ...
  abortTransactionUnless(
        verifyProof(localClientState, remoteClientProof,
...
Preconditions:
  - The two proofs `remoteConnectionProof` and `remoteClientProof` should be correct; this is necessary in connection to properties [ICS3-Proto-2-ConnectionIntegrity] and [ICS3-Proto-3-StateConsistency].
  - Validation of `local` party must pass, and the module running this handler must support at least one of the versions supplied in the input list `remoteVersions`.
                    ...
                    remoteClientProof ClientProof)
{
  ...
  expectedConn = NewConnectionEnd('TRYOPEN', remote, local)
  expectedClient = consensusState(remoteClientProof.height)
  ...
  abortTransactionUnless(remoteClientProof.height <= getCurrentHeight())
  abortTransactionUnless(verifyProof(localClientState, remoteClientProof,
        clientPath(local.clientIdentifier, remoteClientProof.height),
        expectedClient))
...
Preconditions:
  ...
  - The `local` argument is valid (syntactic check), and the `remoteVersion` is supported, i.e., compatible with the local chain -- see `getCompatibleVersions()`
  - The two proofs `remoteConnectionProof` and `remoteClientProof` are syntactically and semantically correct; for guaranteeing properties [ICS3-Proto-2-ConnectionIntegrity] and [ICS3-Proto-3-StateConsistency]
                        ...
                        remoteConnectionProof ConnProof)
{
  ...
  expectedConn = NewConnectionEnd('OPEN', remote, local)
...
Preconditions:
  - The input proof `remoteConnectionProof` is syntactically and semantically correct; for guaranteeing property [ICS3-Proto-2-ConnectionIntegrity].
...
###### Validation
When we talk of validation we usually mean a verification that is done before a handler executes, checking that the `local` connection party is syntactically correct.
In particular, the field `local.connectionIdentifier` is important, similar in functionality with `validateConnectionIdentifier` from [ICS 003](#references).
Additionally, if a `version` or `remoteVersion` input is also present, this should also be validated prior to invoking a handler with that version.
...
This function handles the cryptographic verification of a proof.
It expects four arguments: the local client state, a proof, a path, and the expected data of the proof.
This function is analogous to `verifyMembership` of the [ICS 023](#references), except it must additionally perform two actions: serialize the expected data of the proof, and to extract from the client state the commitment root.

###### `consensusState` and `clientConsensusState`
