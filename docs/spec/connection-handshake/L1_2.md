...
That is, a connection handshake is essentially a three-party protocol: an initiating party which called Alice, a responder party called Bob, and a relayer Y.
...
- **[CH/Seq/1/Agreement]** If the initiator finishes the handshake by establishing a new connection `ci` with the responder, and the responder finishes the handshake obtaining the connection `cr`, then `ci == cr`.
